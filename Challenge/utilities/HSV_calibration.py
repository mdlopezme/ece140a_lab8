...
class DetectCircle(JSONManager):
    ...
    def __init__(self, windowName='Webcam'):
        ...
        if self.calibration_mode:
            ...
            cv.namedWindow('Mask')
            # Create trackbars
            cv.createTrackbar('LowerHue1', 'Mask', self.lower_hue1, 255, self.update_lower_hue1)
            cv.createTrackbar('LowerSat1', 'Mask', self.lower_sat1, 255, self.update_lower_sat1)
            cv.createTrackbar('LowerVal1', 'Mask', self.lower_val1, 255, self.update_lower_val1)
            cv.createTrackbar('UpperHue1', 'Mask', self.upper_hue1, 255, self.update_upper_hue1)
            cv.createTrackbar('UpperSat1', 'Mask', self.upper_sat1, 255, self.update_upper_sat1)
            cv.createTrackbar('UpperVal1', 'Mask', self.upper_val1, 255, self.update_upper_val1)
